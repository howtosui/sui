    ...
    match expected_attr {
        ...
        EA::Parameterized(sp!(_, nm), attrs) => {
            ...
            if expected_failure_kind_vec.len() != 1 {
                // Allow for location-only expected failures
                if let Some((location_loc, location_attr)) = location_opt {
                    let location = convert_location(context, location_loc, location_attr)?;
                    return Some(ExpectedFailure::ExpectedWithError(ExpectedMoveError(
                        StatusCode::ABORTED,
                        None,
