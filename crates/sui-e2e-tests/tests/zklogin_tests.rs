...
async fn zklogin_end_to_end_test() {
    ...
    // 2. wait for current epoch to 8, tx executes ok bc now max_epoch = 10 is within upper bound 2.
    test_cluster
        .wait_for_epoch_with_timeout(Some(8), Duration::from_secs(300))
        .await;
    let tx = build_zklogin_tx(&test_cluster).await;
    let _ = test_cluster
        .wallet
...
async fn zklogin_end_to_end_test_with_auth_state_creation() {
    // set up upper bound for max_epoch to be current epoch + 10.
    let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
        config.set_zklogin_upper_bound_max_epoch(Some(10));
        config
    });
...
/// This builds a zklogin tx where the zkproof is fixed commits to max epoch = 10 and a deterministic ephemeral key.
